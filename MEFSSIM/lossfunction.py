...
def _mef_ssim(X, Ys, window, ws, denom_g, denom_l, C1, C2, is_lum=False, full=False):
    ...
    #muX = F.conv2d(X, window, padding=ws // 2, groups=C).view(K, C, H, W)
    muX_sq = muX * muX
    sigmaX_sq = F.conv2d(X * X, window, padding=ws // 2,
                         groups=C).view(C, H, W) - muX_sq
    
    #sigmaX_sq = F.conv2d(X * X, window, padding=ws // 2,
    #                     groups=C).view(K, C, H, W) - muX_sq
...
class MEFSSIM_Loss(torch.nn.Module):
    def __init__(self, ):
        super(MEFSSIM_Loss, self).__init__()
    ...
    def forward(self, X, Ys):
        ...
        for i in range(b):
            Xi = X[i,:,:,:]
            Ysi = Ys[i,:,:,:]
            loss = 1-self.mefssim(Xi.unsqueeze(0),Ysi.unsqueeze(0)) + loss
        return loss/b
